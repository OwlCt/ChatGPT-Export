...
`index.html` 是一个可静态部署的本地优先 Web 应用。它从本地 `vendor/` 加载 JSZip、pdf-lib 和 fontkit，可以解析导出的 ZIP 文件、缓存已导入内容，并把当前会话导出为文字可选的 PDF。
...
静态阅读器在仓库 `vendor/` 中保存 JSZip `3.10.1`、pdf-lib 和 fontkit；PDF 字体放在 `assets/fonts/`。运行时不需要从 CDN 加载这些依赖。
